...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new SpacedfSDKError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new SpacedfSDKError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const ensurePresent = <T>(value: T | null | undefined): T => {
  if (value == null) throw new SpacedfSDKError(`Expected a value to be given but received ${value} instead.`);
  return value;
};
...
export const coerceInteger = (value: unknown): number => {
  ...
  throw new SpacedfSDKError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new SpacedfSDKError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
